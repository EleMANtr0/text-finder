...
class Interactor(Searcher):
    def __init__(self, directory: str = "", suffix: str = ".txt"):
        super().__init__(directory, suffix)
        self.invoke = []
        root = tk.Tk()
        root.title("Searcher")
    ...
    def update_count(self):
        self.found_count.set(f"{self.match.curr_pos+1}/{len(self.match)}")
    ...
    def search_button(self):
        self.search(self.s.get())
        self.match.cache = 1
        self.invoke = self.next()
        self.label_update(self.invoke)
        self.update_count()
        self.open_button_state(self.invoke[1])
    def left(self):
        self.invoke = self.previous()
        self.label_update(self.invoke)
        self.update_count()
        self.open_button_state(self.invoke[1])
    def right(self):
